...
const Pistolet = {
  ...
  updateStatutReleve: async (releveId, nouveauStatut) => {
    try {
      ...
      const [result] = await db.query('UPDATE releves_postes SET statut = ? WHERE id = ?', [
        nouveauStatut,
        releveId,
      ]);
  ...
  updateStatutPistolet: async (id, statut) => {
    const [result] = await db.query('UPDATE pistolets SET statut = ? WHERE id = ?', [statut, id]);
  ...
  updateIndexFermeture: async (id, index_fermeture) => {
    ...
    try {
      ...
      const [pistolet] = await db.query('SELECT id, dernier_index FROM pistolets WHERE id = ?', [
        id,
      ]);
